...
12 种变异类型覆盖了 **长度变化**（插入、删除）、**值变化**（翻转、算术、边界值替换）、**结构重排**（复制、块交换、随机替换）、**格式保持**（结构感知变异）四个维度，互补探索更广的输入空间。
...
### 5.6 新增结构感知变异（Sample 2 覆盖率优化）★新增

部分被测程序对输入格式有隐含约束。以 `sample2` 为例，有效输入需满足 `前缀.后缀` 且 `int(前缀)` 成功，才能触达 `math.sqrt` 分支。纯随机 havoc 在长 seed 上容易破坏 `.` 分隔符，或无法把字母前缀变成数字。

新增 3 个**结构感知**算子，在变异时尽量保留 `前缀.后缀` 形态：

#### `structure_aware_insert_dot`

- 若输入不含 `.`，在随机位置插入 `.`（空输入则生成 `"N.x"` 形式）
- 若 `.` 已在末尾，补一个后缀字符，避免 `r[1]` 越界
- 若已有合法 `.`，原样返回

#### `structure_aware_prefix_digits`

- 只修改**第一个 `.` 之前**的字节，将其替换为 `0–9`
- 若无 `.`，在开头构造 `"数字." + 原输入`
- 保证 suffix 非空，避免 `split(".")` 后访问 `r[1]` 崩溃

#### `structure_aware_numeric_prefix`

- 用 1–4 位随机数字串**整体替换**第一个 `.` 前的 prefix
- 若无 `.`，生成 `"数字." + 原输入`
- 多位数字时避免 leading zero（如 `"0123"` → 首位改为 `1–9`），提高 `int(r[0])` 成功率

辅助函数 `_split_on_first_dot(s)` 统一在第一个 `.` 处切分 `(prefix, suffix)`。

**效果**：在不修改 `corpus_2` 的前提下，Sample 2 覆盖率由 43.8%（7/16）提升至 50.0%（8/16），成功覆盖第 32 行 `math.sqrt`。

### 5.7 变异器权重机制

```python
self.weights = [10, 5, 10, 10, 10, 12, 12, 8, 8, 8, 10, 10]
```

- 删除类操作（`delete_random_bytes`）权重较低（5），避免输入过快缩小
- havoc 类操作权重略降（12），为结构感知算子留出配额
- 3 个结构感知算子各权重 8–10，在通用探索与格式约束样本之间取得平衡
...
| 6 | `havoc_random_insert` | 随机段插入 | 12 |
| 7 | `havoc_random_replace` | 随机段替换 | 12 |
| 8 | `duplicate_random_bytes` | 复制重复 ★新增 | 8 |
| 9 | `random_block_swap` | 块交换 | 8 |
| 10 | `structure_aware_insert_dot` | 结构感知 · 插入分隔符 ★新增 | 8 |
| 11 | `structure_aware_prefix_digits` | 结构感知 · 数字前缀 ★新增 | 10 |
| 12 | `structure_aware_numeric_prefix` | 结构感知 · 替换数字前缀 ★新增 | 10 |
...
针对结构感知算子的专项验证（`corpus_2` 初始 seed）：

| 指标 | 优化前 | 优化后（30s） |
|------|--------|---------------|
| Sample 2 行覆盖率 | 43.8%（7/16） | **50.0%（8/16）** |
| 新增覆盖行 | — | 第 32 行 `math.sqrt` |
| 执行次数 | ~942k（60s 仍不达标） | ~422k（30s 达标） |

运行命令：

```bash
uv run main.py --sample 2 --run-time 30
```

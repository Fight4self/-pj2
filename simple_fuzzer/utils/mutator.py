...
def _split_on_first_dot(s: str):
    """Split into (prefix, suffix) at the first dot. suffix is empty if no dot."""
    dot = s.find(".")
    if dot == -1:
        return s, None
...
def structure_aware_insert_dot(s: str) -> str:
    """Ensure the input contains a dot separator while keeping most content intact."""
    if not s:
        return f"{random.randint(0, 9)}.x"
...
def structure_aware_prefix_digits(s: str) -> str:
    """Mutate only the segment before the first dot toward decimal digits."""
    prefix, suffix = _split_on_first_dot(s)

    if suffix is None:
        digit = chr(random.randint(ord("1"), ord("9")))
        return f"{digit}.{s}" if s else f"{digit}.x"

    if not prefix:
        prefix = chr(random.randint(ord("1"), ord("9")))
    else:
        data = _to_bytes(prefix)
        count = random.randint(1, len(data))
        for _ in range(count):
            pos = random.randint(0, len(data) - 1)
            data[pos] = random.randint(ord("0"), ord("9"))
        prefix = _from_bytes(data)
...
def structure_aware_numeric_prefix(s: str) -> str:
    """Replace the prefix before the first dot with a fresh numeric token."""
    prefix, suffix = _split_on_first_dot(s)

    num_len = random.randint(1, 4)
    digits = [chr(random.randint(ord("0"), ord("9"))) for _ in range(num_len)]
    if digits[0] == "0" and len(digits) > 1:
        digits[0] = chr(random.randint(ord("1"), ord("9")))
    numeric_prefix = "".join(digits)

    if suffix is None:
        tail = s if s else "x"
        return numeric_prefix + "." + tail
